...
def generic_test(msg):
	...
	state[node_id] = data
    if (node_state == 155):
	print >> sys.stderr, "timer restart failed to restart for a stoped timer on count %d" %data
    if (node_state == 255):
	print "timer restart correctly restarted for a stopped timer on count %d" %data
