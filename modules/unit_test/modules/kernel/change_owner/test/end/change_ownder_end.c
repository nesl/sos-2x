...
#define MSG_MOD_REMOVED (MOD_MSG_START + 4)
		...
		case MSG_TEST_ADDR:
			{
			  ...
			  new_addr = (addr_msg_t*) sys_msg_take_data(msg);
				s->test_ptr = new_addr->pt;
				sys_change_own(s->test_ptr);
				
				sys_timer_start(TEST_APP_TID, TEST_APP_INTERVAL, SLOW_TIMER_REPEAT);
				s->state = TEST_APP_FINAL;

				send_new_data(101, OTHER_PID);
			}
			break;

		case MSG_MOD_REMOVED:
			{
		...
		case MSG_TIMER_TIMEOUT:
			{
              ...
              uint8_t *temp;
							...
							temp = (uint8_t*) sys_malloc(sizeof(uint8_t)*10);
