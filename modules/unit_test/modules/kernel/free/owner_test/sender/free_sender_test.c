...
#define MSG_ATTEMPT_FREE (MOD_MSG_START + 2)
#define MSG_FREE_ATTEMPTED (MOD_MSG_START + 3)
		...
		case MSG_TIMER_TIMEOUT:
			{
				switch(s->state){
				  case TEST_APP_INIT:
					  {
							free_test_t *data_msg;

							data_msg = (free_test_t *) sys_malloc(sizeof(free_test_t));
							data_msg->d = (uint32_t *) sys_malloc(sizeof(uint32_t));

							s->pt = data_msg->d;
							*(data_msg->d) = 0x12345678;
