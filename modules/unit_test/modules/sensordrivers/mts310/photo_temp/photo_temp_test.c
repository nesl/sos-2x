...
#define DRIVER_0_ID MTS310_TEMP_SID
#define DRIVER_1_ID MTS310_PHOTO_SID
...
static int8_t generic_test_msg_handler(void *state, Message *msg)
{
	...
	switch ( msg->type ) {
		...
		case MSG_FINAL:
			...
			sys_sensor_disable(DRIVER_1_ID);
			  ...
			  //sys_led(LED_YELLOW_TOGGLE);
				switch (s->state) {
					...
					case TEST_APP_TEMP:
						...
						s->state = TEST_APP_TEMP_BUSY;
						sys_sensor_get_data(DRIVER_0_ID);
						break;

					case TEST_APP_TEMP_BUSY:
						break;

					case TEST_APP_PHOTO:
						if ( sys_sensor_enable(DRIVER_1_ID) != SOS_OK)
							sys_timer_stop(TEST_APP_TID);

						s->state = TEST_APP_PHOTO_BUSY;
		...
		case MSG_DATA_READY:
			{
				...
				switch(s->state) {
					case TEST_APP_TEMP_BUSY:
						s->state = TEST_APP_PHOTO;
						break;
					case TEST_APP_PHOTO_BUSY:
						s->state = TEST_APP_TEMP;
						break;
				}
			}
