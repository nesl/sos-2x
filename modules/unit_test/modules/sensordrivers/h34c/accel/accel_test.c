...
#define DRIVER_0_ID H34C_ACCEL_0_SID
#define DRIVER_1_ID H34C_ACCEL_1_SID
#define DRIVER_2_ID H34C_ACCEL_2_SID
...
static int8_t generic_test_msg_handler(void *state, Message *msg)
{
	...
	switch ( msg->type ) {
		...
		case MSG_TIMER_TIMEOUT:
			  ...
			  //sys_led(LED_YELLOW_TOGGLE);
				switch (s->state) {
					...
					case TEST_APP_ACCEL_0:
						...
						if (sys_sensor_get_data(DRIVER_0_ID) != SOS_OK)
          ...
          case TEST_APP_ACCEL_2:
						s->state = TEST_APP_ACCEL_2_BUSY;
						sys_sensor_get_data(DRIVER_2_ID);
						break;

					case TEST_APP_ACCEL_2_BUSY:
						break;
				}
			}
			break;
				  ...
				  sys_led(LED_GREEN_TOGGLE);
					...
					switch(s->state) {
						case TEST_APP_ACCEL_0_BUSY:
							...
							data_msg->sensor_num = 0;
							break;
						case TEST_APP_ACCEL_1_BUSY:
							s->state = TEST_APP_ACCEL_2;
							data_msg->sensor_num = 1;
							break;
						case TEST_APP_ACCEL_2_BUSY:
							s->state = TEST_APP_ACCEL_0;
							data_msg->sensor_num = 2;
							break;
					}
