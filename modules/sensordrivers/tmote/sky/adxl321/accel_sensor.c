...
// Sensor board specific configuration file.
// It should also be included in the application modules
// that intend to use the bandpass filters on board.
#include "adxl321_sensor.h"

#define LED_DEBUG
#include <led_dbg.h>

// Number of sensors (channels) handled by this driver.
// Accelerometer: X, Y axis - Original
// Accelerometer: X, Y axis - Filtered
		...
		case SENSOR_ENABLE_COMMAND: {
			switch(s->state) {
				case DRIVER_ENABLE: {
					...
					if (filter == FILTER_NONE) return SOS_OK;
					// Set all pins to LOW initially.
					P2OUT &= ~(BV(0)|BV(3)|BV(6));
					if (filter & 1) {
						P2OUT |= BV(0);
					}
					if (filter & 2) {
				...
				case DRIVER_DISABLE: {
					...
					// Set I/O function and direction to output for GIO[0-3]
					// GIO[1] controls SHDN pin
...
int8_t accel_sensor_msg_handler(void *state, Message *msg) { 
	...
	switch (msg->type) {
		case MSG_INIT: {
			...
			s->map[2].sensor = ACCEL_X_FILTER;
			s->map[2].channel = ADC_DRIVER_CH6;
			s->map[3].sensor = ACCEL_Y_FILTER;
			s->map[3].channel = ADC_DRIVER_CH7;
			...
			ret = sys_sensor_driver_register(ACCEL_X_FILTER, SENSOR_CONTROL_FID);
			if (ret < 0) {
				sys_sensor_driver_deregister(ACCEL_X_SENSOR);
			...
			ret = sys_sensor_driver_register(ACCEL_Y_FILTER, SENSOR_CONTROL_FID);
			if (ret < 0) {
				sys_sensor_driver_deregister(ACCEL_X_SENSOR);
				sys_sensor_driver_deregister(ACCEL_Y_SENSOR);
				sys_sensor_driver_deregister(ACCEL_X_FILTER);
				s->state = DRIVER_ERROR;
				return -EINVAL;
			}
			...
			if (ret < 0) {
				...
				sys_sensor_driver_deregister(ACCEL_Y_SENSOR);
				sys_sensor_driver_deregister(ACCEL_X_FILTER);
				sys_sensor_driver_deregister(ACCEL_Y_FILTER);
