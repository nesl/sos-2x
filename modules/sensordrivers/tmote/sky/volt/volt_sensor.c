...
static int8_t volt_sensor_data_ready_cb(func_cb_ptr cb, adc_feedback_t fb, sos_pid_t app_id, 
							uint16_t channels, sensor_data_msg_t* buf);
// control function registered with sensor API
static int8_t volt_sensor_control(func_cb_ptr cb, sensor_driver_command_t command, 
			sos_pid_t app_id, sensor_id_t sensor, sample_context_t *param, void *context);
// Sensor control feedback from the ADC driver
static int8_t volt_sensor_feedback(func_cb_ptr cb, sensor_driver_command_t command, 
						uint16_t channel, void *context);

static int8_t volt_sensor_msg_handler(void *state, Message *msg);

static const mod_header_t mod_header SOS_MODULE_HEADER = {
	mod_id : VOLT_SENSOR_PID,
	code_id : ehtons(VOLT_SENSOR_PID),
  	state_size : sizeof(volt_sensor_state_t),
	num_timers : 0,
	num_sub_func : 0,
	num_prov_func : 3,
	platform_type : HW_TYPE,
	processor_type : MCU_TYPE,
	module_handler : volt_sensor_msg_handler,
	funct : {
		{volt_sensor_control, "cCw2", VOLT_SENSOR_PID, SENSOR_CONTROL_FID},
		{volt_sensor_data_ready_cb, "cCS3", VOLT_SENSOR_PID, SENSOR_DATA_READY_FID},
		{volt_sensor_feedback, "cCw2", VOLT_SENSOR_PID, SENSOR_FEEDBACK_FID},
	},
};
...
static int8_t volt_sensor_control(func_cb_ptr cb, sensor_driver_command_t command, 
			sos_pid_t app_id, sensor_id_t sensor, sample_context_t *param, void *context) {
	volt_sensor_state_t *s = (volt_sensor_state_t *)sys_get_state();
...
int8_t volt_sensor_msg_handler(void *state, Message *msg) { 
	volt_sensor_state_t *s = (volt_sensor_state_t *)state;

	switch (msg->type) {
		case MSG_INIT: {
			// Setup the sensor <-> channel mapping.
			s->map[0].sensor = INTERNAL_VOLTAGE_SENSOR;
			s->map[0].channel = ADC_DRIVER_CH11;
			...
			int8_t ret = sys_sensor_driver_register(INTERNAL_VOLTAGE_SENSOR, SENSOR_CONTROL_FID);
			if (ret < 0) {
			...
			if (ret < 0) {
				...
				sys_sensor_driver_deregister(INTERNAL_VOLTAGE_SENSOR);
