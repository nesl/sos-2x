...
static const mod_header_t mod_header SOS_MODULE_HEADER = {
	...
	funct : {
		...
		{par_sensor_feedback, "cCw2", LITEPOT_PID, SENSOR_FEEDBACK_FID},
	},
};
...
int8_t par_sensor_msg_handler(void *state, Message *msg) { 
	...
	switch (msg->type) {
		case MSG_INIT: {
			...
			s->map[0].sensor = LIGHT_PAR_SENSOR;
			s->map[0].channel = ADC_DRIVER_CH4;
			...
			int8_t ret = sys_sensor_driver_register(LIGHT_PAR_SENSOR, SENSOR_CONTROL_FID);
			if (ret < 0) {
			...
			if (ret < 0) {
				...
				sys_sensor_driver_deregister(LIGHT_PAR_SENSOR);
