...
static int8_t tsr_sensor_data_ready_cb(func_cb_ptr cb, adc_feedback_t fb, sos_pid_t app_id, 
							uint16_t channels, sensor_data_msg_t* buf);
// control function registered with sensor API
static int8_t tsr_sensor_control(func_cb_ptr cb, sensor_driver_command_t command, 
			sos_pid_t app_id, sensor_id_t sensor, sample_context_t *param, void *context);
// Sensor control feedback from the ADC driver
static int8_t tsr_sensor_feedback(func_cb_ptr cb, sensor_driver_command_t command, 
...
static const mod_header_t mod_header SOS_MODULE_HEADER = {
	...
	funct : {
		...
		{tsr_sensor_feedback, "cCw2", PHOTOTEMP_SENSOR_PID, SENSOR_FEEDBACK_FID},
	},
};
...
static int8_t tsr_sensor_control(func_cb_ptr cb, sensor_driver_command_t command, 
			sos_pid_t app_id, sensor_id_t sensor, sample_context_t *param, void *context) {
	tsr_sensor_state_t *s = (tsr_sensor_state_t *)sys_get_state();
...
int8_t tsr_sensor_msg_handler(void *state, Message *msg) { 
	...
	switch (msg->type) {
		case MSG_INIT: {
			...
			s->map[0].channel = ADC_DRIVER_CH5;
