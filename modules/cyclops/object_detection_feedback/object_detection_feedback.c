...
#include "object_detection_feedback.h"
      ...
      case OBJECT_DETECTION_TID:
		{
		  ...
		  if (s->state != IMAGER_PREPARED) return -EINVAL;
  ...
  case SNAP_IMAGE_DONE:
    {
	  ...
	  // if this is a zoomed image, just send it
	  if(s->zoom_state == 1) {
		  // Send the picture
#ifdef USE_SERIAL_DUMP
      ...
      if (s->firstImage == 1) {	//first image collected, backMat should equal image	
		...
		if (overTheThresh > DETECT_THRESH) {
		  // Set the parameters for zooming in
		  s->capture->offset.x = -(s->capture->inputSize.x/2) 
			+ (maxcol * s->capture->inputSize.x / s->img->size.x);
		  s->capture->offset.y = -(s->capture->inputSize.y/2) 
			+ (maxrow * s->capture->inputSize.y / s->img->size.y);	
		  s->capture->inputSize.x = ROWS;
		  s->capture->inputSize.y = COLS;
		  ker_setCaptureParameters(s->capture);
		  s->zoom_state = 1;
		}
...
#ifndef _MODULE_
mod_header_ptr object_detection_feedback_get_header()
{
